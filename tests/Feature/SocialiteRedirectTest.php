...
it('redirects to home with alt_id uuid on socialite callback success', function () {
    ...
    expect($query['user_id'])->toBe($signee->alt_id)
        ->and($query['user_id'])->not->toBe((string) $signee->id);
});
...
test('normalizes pkcs1 private key and generates apple jwt token successfully', function () {
    $res = openssl_pkey_new([
        "private_key_type" => OPENSSL_KEYTYPE_EC,
        "curve_name" => "prime256v1",
    ]);
